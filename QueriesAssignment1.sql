...
INSERT INTO Books (Book_ID, Title, Author_ID, ISBN, Publication_Year, Available_Copies)
VALUES (2, 'The Hobbit', 2, '9780261103344', 2001, 1);
...
SELECT Book_ID, Title, Author_ID FROM Books WHERE Available_Copies =1;

2. List Author whose name starts with specific letter
SELECT * FROM Authors WHERE Author_Name LIKE 'J%';

3. List menbers with no address recorded
SELECT Member_ID, Member_Name FROM Members WHERE Address IS NULL;
...
SELECT Book_ID, Title, Author_ID FROM Books WHERE Publication_Year >2000;

6. List Borrowing with no fines
SELECT * FROM Borrowings WHERE Fine =0;

7. List members sorted by membership dates in decresing order
...
9. List books with title containing a specific word
SELECT * FROM Books WHERE Title LIKE '%Potter%';

10. List borrowing returned on a specified date
SELECT * FROM Borrowings WHERE Return_Date = TO_DATE('2025-10-14','YYYY-MM-DD');
...
14. List borrowing with due date in a specific month
